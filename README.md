...
- **Bounds checking scope**: covers constant-bound arrays, dynamically-bound *local* arrays, and array *parameters*, checked per dimension. The one uncovered case is a dynamic class array *field* passed as an argument: its size can't be recovered at the call site, so it forwards an "unknown" marker and is left unchecked rather than risk a false abort.
